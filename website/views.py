...
def index(request):
    ...
    if request.POST:
        date = "-".join(
            [request.POST["year"], request.POST["month"], request.POST["day"]]
        )
    ...
    month = [
        {"month": i, "current": i == int(datetime.now().strftime("%m"))}
        for i in range(1, 13)
    ]
    ...
    if request.POST:
        ...
        payment = E.PmtInf(
            ...
            E.Cdtr(
                E.Nm(pay[0]["name_project"]),
                E.PstlAdr(
                    E.Ctry("SI"), E.AdrLine("Planina 3"), E.AdrLine("4000 Kranj")
                ),
            ),
            E.CdtrAcct(
            ...
            E.ChrgBr("SLEV"),
            E.CdtrSchmeId(
                E.Id(E.PrvtId(E.Othr(E.Id(theId), E.SchmeNm(E.Prtry("SEPA")))))
            ),
        )
        for p in pay:
            if "record-transaction" in request.POST:
                sql = (
                    "UPDATE agreement_pay_installment "
            ...
            TxInf = E.DrctDbtTxInf(
                E.PmtId(
                    E.InstrId(
                        "SI00" + p["id_agreement"][:8] + "-" + p["id_agreement"][8:]
                    ),
                    E.EndToEndId(
                        "SI00" + p["id_agreement"][:8] + "-" + p["id_agreement"][8:]
                    ),
                ...
                E.RmtInf(
                    E.Ustrd(
                        "DONACIJA %s-%s"
                        % (str(p["id_agreement"]), str(p["id_vrstica"]))
                    )
                ),
            )
        ...
        xml = xml.replace(
            "xmlns__xsi", "xmlns:xsi"
        )  # I can't seem to set this attrib normaly

        filename = "db-%s-%s-%s.xml" % (
...
def invoices(request):
    ...
    if request.POST:
        ...
        return render_to_response(
            "invoices.html", data, context_instance=RequestContext(request)
        )
    return render_to_response(
        "invoices.html", data, context_instance=RequestContext(request)
    )
...
def invoices_export(request):
    ...
    cursor.execute(
        """SELECT DISTINCT 
         ...
         ORDER BY a.id_agreement, api.id_vrstica
        """.format(",".join("%s" for _ in installments)),
        installments,
    )
    rows = cursor.fetchall()
    invoices = []
    ...
    credentials = ServiceAccountCredentials.from_json_keyfile_name(
        "donator-1500c8e231aa.json", scopes
    )
    ...
    drive_request = drive.files().export_media(
        fileId=file_id,
    ...
    if is_store_date:
        ...
        cursor.execute(
            ...
            WHERE id_vrstica IN ({}) 
        """.format(",".join("%s" for _ in installments)),
            [store_date] + installments,
        )
        cursor.execute("COMMIT")
    return render_to_response(
        "invoices_done.html",
        {"file_id": file.get("id"), "file_name": filename},
        context_instance=RequestContext(request),
    )
